...
const headerTag = document.querySelector("header a.nameFarsi");
...
$.ajax({
    ...
    success: function(data) {
        for (var i=0; i<data.length; i++) {
            var row = $('<tr data-style="' + data[i].styleclass + '" class="inactive ' + data[i].category + '"id="'+ i +'"><td class="date">' + data[i].date + '</td><td class="category">' + data[i].category + '</td><td class="title"><a>' + data[i].title + '<a/><div class="gallery"></div></td><td class="colab">' + data[i].colab +'<iframe src="'+ data[i].iframe +'"></iframe><div class="longtext"><p>'+ data[i].longtext +'</p></div></td><td class="place">' + data[i].place + '</td><td class="detail">' + data[i].detail + '</td></tr>');
